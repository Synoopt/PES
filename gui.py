...
基于 Streamlit 的简易图形界面：训练、可视化与分子模拟。
"""
...
with st.sidebar:
    ...
    lang_label = t("zh", "language")  # label itself双语
...
with TAB_TRAIN:
    ...
    if st.button(t(lang_code, "start_train"), type="primary"):
        try:
            ...
            # 自动生成目录名（含时间戳）与文件名（同样含时间戳）
            stem = f"{cfg['num_layers']}-{cfg['hidden_dim']}-{cfg['activation_function']}"
            tag = datetime.now().strftime("%Y%m%d-%H%M%S")
            out_dir = f"{stem}-{tag}"         # 目录也带时间戳
            ensure_dir(out_dir)

            cfg["save_model_path"] = f"{stem}-{tag}.pth"   # 文件名与目录名一致
        ...
        try:
            if not auto_dir or not auto_model_path or not os.path.exists(auto_model_path):
                ...
                with st.spinner(t(lang_code, "sim_running")):
                    # 兼容 run_simulation 可能使用 cfg['save_model_path'] 的情况：
